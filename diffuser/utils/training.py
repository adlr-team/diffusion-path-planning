...
class Trainer(object):
    ...
    def train(self, n_train_steps):
        ...
        for step in range(n_train_steps):
            ...
            if (
                ...
                end_tensors = []
                for _ in range(4):
                    start_tensor, end_tensor = self.generate_random_tensors(1)
                    start_tensors.append(start_tensor)
                    end_tensors.append(end_tensor)


                self.render_samples_list(self.renderer.env, n_samples=1,
                                        start_tensors= start_tensors,
                                        end_tensors = end_tensors)

            self.step += 1
        wandb.log({"Time per episode": timer()})


    def generate_random_tensors(self, batch_size):
        start_tensor = torch.zeros((batch_size, 2))
        end_tensor = torch.zeros((batch_size, 2))
        
        start_tensor[:, 0] = -3 + torch.rand(batch_size)  # Sample between 2 and 3 for the first value
        start_tensor[:, 1] = -3 + torch.rand(batch_size)  # Sample between -3 and -2 for the second value
        
        end_tensor[:, 0] = 2 + torch.rand(batch_size)  # Sample between -3 and -2 for the first value
        end_tensor[:, 1] = 2 + torch.rand(batch_size)  # Sample between 2 and 3 for the second value
        
        return start_tensor.cpu(), end_tensor.cpu()
    ...
    def render_samples(
        ...
        for i in range(batch_size):
            ...
            savepath_collision = os.path.join(self.logdir, f"sample-collisions-{self.step}-{i}.png")
            if render_reference:
            ...
            if collision_count > 0:
                print(f"Collision observations:")
                self.renderer.composite(savepath_collision, collisions_obs, env=env)



    def render_samples_starting(
        self,
        env,
        batch_size=1,
        n_samples=1,
        get_cond_from_env=False,
        render_reference=False,
        start_tensor= torch.tensor([0.0, 0.0]),
        end_tensor = torch.tensor([0.0, 0.0])
    ):
        """
        renders samples from (ema) diffusion model
        Note: set get_cond_from_env to True if you want to get the conditions from the environment
    ...
    def render_samples_list(
            self,
            env,
            n_samples=1,
            start_tensors= [torch.tensor([0.0, 0.0])],
            end_tensors = [torch.tensor([0.0, 0.0])]
        ):        
            observations_list = []
            collision_counts = 0
            for i in range(len(start_tensors)):
                ...
                # We have to normalize the conditions before passign them to the model.
                conditions[0] = self.dataset.normalizer.normalize(
                    conditions[0].cpu(), "observations"
                )
                conditions[255] = self.dataset.normalizer.normalize(
                    conditions[255].cpu(), "observations"
                )
                # Is this wrong?
                normed_c = conditions[0]
                conditions = to_device(conditions, self.device)
                ...
                # observations = blocks_add_kuka(observations)
                ####
                savepath = os.path.join(self.logdir, f"sample-sameontop-{self.step}-{i}.png")
                # if render_reference:
                #     print("The trajectory from the dataset:")
                ...
                # print(f"Collision_count_diffusion_model : {collision_count}")
                #wandb.log({"Collision count:": collision_count})
                savepath_collision = os.path.join(self.logdir, f"sample-collisions-corners-{self.step}-{i}.png")
            ...
            print(f"Collision_count_diffusion_model average corners: {collision_counts / len(start_tensors)}")
            wandb.log({"Collision count from corners:": collision_counts / len(start_tensors)})
            ...
            self.renderer.renders_list(savepath, observations_list, env=env)
                # print(f"Collision observations: {collisions_obs}")
                # if collision_count > 0:
                #     print(f"Collision observations:")
                #     self.renderer.composite(savepath, collisions_obs, env=env)