...
def download_hdf5(url, filename):
    try:
        response = requests.get(url)
        print("Creating folder")
        file_path = os.path.join("diffuser/datasets", filename)
...
# dataset = "maze2d-large-dense-v1.hd5f"
# download_hdf5(DATASET_URLS[dataset.rsplit(".", 1)[0]], dataset)


def hdf5_to_dataframes(filepath):
...
# Load data
# file_path = os.path.join("diffuser/datasets", dataset)
# dataframes = hdf5_to_dataframes(file_path)
